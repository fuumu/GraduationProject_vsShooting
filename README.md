...
<div>
  <video controls src="https://raw.githubusercontent.com/nRen28/GraduationProject_vsShooting/ren/img/play_singlemode.mp4" muted="false"></video>
</div>
...
ブザーの音量の調整はポテンショメータで行うことができます。  
音量はボリュームを**時計回り**に回すと**大きく**、**反時計回り**に回すと**小さく**なります。  

#### スコアのリセット

ゲーム起動時に**12番ピンをGNDに接続**しておくことで、**シングルモードのハイスコアを初期化**することができます。  
この処理は、**電源投入時**または**リセット時**のタイミングでピンの状態を確認し、**LOW（GNDに接続）**であればスコアのリセットを実行します。  
